document.addEventListener("DOMContentLoaded", function() {
    ...
    async function bufferNextTracks(tracks) {
        ...
        // Mobile Network Throttling: Aggressive background caching kills mobile data connections and blocks API calls.
        // We limit pre-buffering to just the currently playing song + the literal next song in the queue if on mobile.
        const cacheLimit = isMobile ? 2 : 4;
        ...
        // Only pre-cache random "emergency" tracks if we are on a strong desktop connection
        if (!isMobile && catData && catData.length > 0) {
